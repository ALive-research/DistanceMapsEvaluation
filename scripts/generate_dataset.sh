...
for i in ${LIVER_DATASET}; do
    echo "Processing ${i}"
    ${LABELEXTRACT_OPERATOR} -l 1 -L 1 -i ${i} -o /tmp/label_tmp.nrrd
    ${CONVERT_OPERATOR} -i /tmp/label_tmp.nrrd -d 0 -u -c -o ${1}/liver_$(printf %03d $livernr).nrrd
    livernr+=1
done
...
for i in ${VESSELS_DATASET}; do
    echo "Processing ${i}"
    ${LABELEXTRACT_OPERATOR} -l 1 -L 1 -i ${i} -o /tmp/label_tmp.nrrd
    ${CONVERT_OPERATOR} -i /tmp/label_tmp.nrrd -d 0 -u -c -o ${1}/vessels_$(printf %03d $vesselnr).nrrd
    vesselnr+=1
done
...
for i in ${ALL_DATASET}; do
    echo "Processing ${i}"
    ${LABELEXTRACT_OPERATOR} -l 2 -L 1 -i ${i} -o /tmp/label_tmp.nrrd
    ${CONVERT_OPERATOR} -i /tmp/label_tmp.nrrd -d 0 -u -c -o ${1}/tumor_$(printf %03d $tumornr).nrrd
    tumornr+=1
done
